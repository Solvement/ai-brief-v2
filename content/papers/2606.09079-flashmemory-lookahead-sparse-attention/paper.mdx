...
- **总平均**:FM-DS-V4 准确率 77.5 vs 基线 76.9(+0.6);Table 1 的 Avg 行物理 KV cache 是 **0.42 GB vs 3.90 GB**(原表逐字:基线 "76.9 (3.90 GB)"、本法 "77.5 (0.42 GB)";直接相除约 10.8%)。原文 §3.2 文字另把逐基准内存占比平均汇总为**13.5%**(即平均减少 86.5%),两者是不同口径——一个是绝对量平均的比值,一个是各基准占比的平均[^t1]。
...
- **越长越赚**:LongBench-v2-L(493K)70.0 vs 68.1(+1.9),物理 KV cache **0.74 vs 7.52 GB**(原表逐字:"68.1 (7.52 GB)" vs "70.0 (0.74 GB)",约 9.8%);LongMemEval-M(500K)40.2 vs 39.3,**0.70 vs 7.61 GB**(约 9.2%)。
...
- **对抗测试(No-Context)**:历史完全无关的题,精度保持(LongMemEval-S 95.0 / LongMemEval-M 92.5 vs 基线 96.7 / 91.2,原文 Table 2),但**没拿到理想的 O(1) 内存**——500K 时占比降到 8.4%,绝对块数却比 125K 时涨了约 2.5 倍:Sigmoid 在超大干扰池里持续泄漏假阳性。块的 token 数原文未披露[^s331]。
...
[^t1]: 原文 §3.1 实验设置与 §3.2 Table 1;KV 开销由 sglang 部署日志测得(8×H20)。逐格数值以原文 Table 1 为准——Avg. 行逐字为 "76.9 (3.90 GB)"(DS-V4-Flash) vs "77.5 (0.42 GB)"(FM-DS-V4),直接相除约 10.8%;13.5% 是原文 §3.2 文字给出的逐基准占比平均,两种口径。denoiser 机制解释见 §3.2("expert attention denoiser, filtering out thousands of irrelevant historical chunks")。
