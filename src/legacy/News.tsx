...
function sourceTypeClass(t: string): string {
  ...
  return "community"; // reddit + community
}
...
export function News() {
  ...
  if (err) {
    return (
      <>
        ...
        <main className="page"><div className="notice error">加载资讯数据失败：{err}</div></main>
      </>
    );
  }
  ...
  if (!data) {
    return (
      <>
        <SiteHeader active="news" />
        <main className="page"><div className="loading">正在加载资讯...</div></main>
      </>
    );
  }
      ...
      <main className="page news-page">
        <section className="models-intro">
          <div>
            ...
            <p>聚合官方公告、媒体报道与社区热帖。按日期倒序排列，点标题跳到原文。</p>
          </div>
        </section>

        {groups.length === 0 ? (
          <div className="notice">暂无资讯。</div>
        ) : (
          ...
          {groups.map((group) => (
            <section className="news-day" key={group.key}>
              <div className="news-day-head">
                ...
                <span className="news-day-count">{group.items.length} 条</span>
              </div>
                ...
                {group.items.map((item, i) => (
                  ...
                  >
                    <div className="news-row-main">
                      ...
                      {item.summary && <div className="news-row-summary">{item.summary}</div>}
                      <div className="news-row-meta">
                        <span className={`news-source-badge ${sourceTypeClass(item.sourceType)}`}>
                          {SOURCE_TYPE_LABEL[item.sourceType] || "来源"}
                        </span>
