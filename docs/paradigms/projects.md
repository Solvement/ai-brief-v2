...
- **不精读但有亮点（核心决策 Kevin 2026-06-11）**：对 Kevin 学习/AI 进化帮助不大的高 star 项目（多为通用工具/非 agent 架构/纯 skill）**不进 deep/standard 精读**（省深读成本），但**必须有 light 亮点卡**——回答"它凭什么火"。判断标准=对"外脑方法论沉淀"有没有用：有用→精读沉淀；没用→只写亮点让 Kevin 扫一眼知道发生了什么。**不因为火就精读，也不因为不精读就不展示。**
