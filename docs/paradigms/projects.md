...
Tier 3 深度（在 Tier2 上加）：· 它怎么工作：核心机制+一个非专家能复述的类比 · 为什么和同类本质不同：讲设计取舍不是功能罗列 · 对从业者意味着什么：能解决读者手上什么问题 · 交叉链接：若有配套论文/模型，链到「文章」「模型」栏目 · **core_concepts**：3-5 个**承重**概念 `{name, role: primary|supporting, evidence}`，每个须出现在 README/代码（不是营销词）——喂 Mind Palace 核心概念门，paper↔project 判边的项目侧锚（KG-2, 2026-06-09） · **claim_ledger**：项目关键 claim 各带 `supports / does_not_support / threat` 三栏（supermemory 深读首创结构，上提为 Tier 3 标准件）
