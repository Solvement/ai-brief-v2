...
【v3 变更说明（2026-06-12 凌晨，Kevin 原话级）——精读的定义被重写】
**精读 = 看过我们的分析之后，读者要达到足以复现这个项目的水平。** 现状病：大部分篇幅花在风险/不确定性，项目本身的优势、架构思路、如何实现完全没讲——这是跑偏，立即纠正。深读正文按下面「复现级四问」组织，**优势和实现为主，缺点只在影响判断时一笔带过**（dependency_platform_risk / unknowns 两节合计不得超过全文 15%）。

**复现级四问（Tier3/deep 正文的骨架，按此顺序展开）：**
1. **它在打谁/替谁（生态锚点对比，必写）**——读者脑子里本来就有锚点（一想到 Hermes 就想到 OpenClaw）。先找出这个项目的 1-2 个**显而易见的对位物**（同生态/同品类/它声称取代的东西），正面回答：为什么它比对位物好？区别在哪？**"为什么火"的一半就是这个对比**——不接锚点 = 没回答读者的第一个问题。对位物找法：README 自己点名的、HN/Reddit 讨论里反复并提的、同 track 里 star 最高的。
2. **它凭什么赢（机制级，复现深度）**——架构是什么、模块怎么分、多 agent 怎么编排、harness 怎么搭、数据怎么流、关键设计决策**为什么这么做而不是那样做**。标准：看完能画出架构图+说出每个模块职责+复述关键取舍。必须**真实读过仓库源码/架构文件**再写（clone 或 GitHub 读核心目录），不许从 README 摘要反推。配 Mermaid 架构图。
3. **哪些可拆可换可偷（可替换性分析）**——主动问：这个项目哪层能换（WebUI 能不能换成别的前端）、哪层是真壁垒、壳子/底座好在哪（oh-my-X 类项目的洞察=Claude Code 壳与内核模型可分离，所以能接任意 API——为什么可分离？怎么做到的？）。这种思考直接产 Mind Palace 的 composes_with/improves_on 边和「怎么利用」——**分析和连边是同一种思维**。
4. **为什么是现在火（归因）**——接 trending_cause 管线证据（release 事件/HN 热帖/生态缺口/名人带货），一段讲清时机。

**连带纪律**：配套项目（如 Hermes ↔ Hermes-WebUI）分析时必须互相引用并显式写出关系——这类显而易见的生态边漏连是关系引擎的事故级缺陷。
