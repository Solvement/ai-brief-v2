...
证据:通过引用 ICLR 2025 论文（100% 攻击成功率）和微软红队经验支撑了提示词防御不可靠的观点。。边界:未提供 AGT 自身拦截效果的 benchmark，仅提供了原理性论证。。风险:虽然原理正确，但若策略引擎本身有漏洞（如逃逸条件），该‘确定性’仍可被绕过。。See [[content/agent-governance-toolkit]]。
