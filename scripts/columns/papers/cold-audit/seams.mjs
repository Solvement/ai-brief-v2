...
2. faithful(忠实):无原文不支持的话;自报 vs 实测分清;无编造数字。任一编造 = 重大。
   **数字指控的引证纪律(2026-06-10 加固,防审稿幻觉)**:指控「数字与原文不符」判 major 前,必须在 gap 里附上
   你在上方源文里**逐字找到的那一行/那一格**(≤30 字引文即可)。源文里找不到可逐字引用的对应行 → 你可能看错了
   列/行(同表常有 Recency/Random 等基线列),此时**只许 severity=minor 并在 gap 标「[未能逐字核出,待人查]」**,
   不许 major。一次幻觉指控会让正确的深读被错杀并诱导作者把对的数字改错——代价高于漏报。
